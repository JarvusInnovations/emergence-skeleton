 ...
 * <li>5. July 2010 - chris@devnuts.com - implemented record.searchFormat()</li>
 * </ul>
 * @version   1.1.1
...
declare('Ext::ux::grid::Search', function(use) {
    use(['Ext::ux::util'] , function(use) {
					...
					if(val) {
						store.filterBy(function(r) {
							...
							this.menu.items.each(function(item) {
								if(!item.checked || retval || !item.dataIndex) {
									return;
								}
								var f = r.fields.get(item.dataIndex)
								var rv = r.get(item.dataIndex);
								rv = rv instanceof Date ? rv.format(this.dateFormat || r.fields.get(item.dataIndex).dateFormat) : rv;
								rv = f.searchFormat ? f.searchFormat(rv) : rv;
