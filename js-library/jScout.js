...
/**
 * Patched by chris@devnuts.com with inspiration from http://www.extjs.com/forum/showthread.php?t=40866&page=3
 * to load CSS files is isLoaded and return true without queing them
 */
...
jScout.packageMgr = function () {
    ...
    return {
        ...
        getPackage : function(name) {
            if (!namedPackages[name]) {
            	namedPackages[name] = new jScout.packageObj( { name: name } );
            	//console.log('Created new package %s', name);
            }
            else
            {
            	//console.log('Got package from cache %s', name);
            }
...
jScout.packageObj = Ext.extend(Ext.util.Observable,{
    ...
    load : function (sync) {
        ...
        this.loading = true;
        //console.log('Loading %s from %s', this.name, this.getUrl())
