 ...
 class ExtJS
{
	...
	static public function getRecordMetadata($recordClass)
	{
		$metadata = array(
			'root' => 'data'
	...
	static public function getFieldTypeConfig($field)
	{
		switch($field['type'])
		{
			case 'integer':
	...
	static public function getColumnModelConfig($recordClass)
	{
		$columns = array();
	...
	static public function getColumnHeader($field)
	{
		return !empty($field['label']) ? $field['label'] : Inflector::spacifyCaps($field['columnName']);
	}
	
	
	static public function getJson($value, $indent = 0)
	{
		if(is_string($value))
		{
