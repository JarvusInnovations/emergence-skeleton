...
class Emergence_FS
{
	...
	static public function importFile($sourcePath, $destinationPath)
	{
...
#				print("Deleted file $fileInfo[ID] at $path<br>");
    ...
    static public function getCollectionLayers($path, $localOnly = false)
    {
        // split path into array
		if(is_string($path)) {
			$path = Site::splitPath($path);
    ...
    static public function findFiles($filename, $useRegexp = false, $scope = null, $localOnly = false)
	{
        $collections = array();
    ...
    static public function getAggregateChildren($path)
    {
        $children = array();
