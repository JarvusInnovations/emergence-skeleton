...
class Format
{
	...
	static public function usPhone($input, $format = '(%s) %s-%s')
	{
		// strip non-digits
		...
		if (strlen($numbers) != 10) return $input;
	...
	static public function truncate($string, $limit=300, $break = '.', $pad = '...')
	{
	...
	function fuzzyTime($timestamp) {
      ...
      */
        ...
        else if($diff < 2592000) { //Weeks
            $val = round(($diff / 604800));
            $unit = 'week';
        }
        else if($diff < 77760000) { //Months (30 Days)
	...
	static public function micsText($text, $mode = 'format')
	{
	...
	static public function twitterText($text) {
