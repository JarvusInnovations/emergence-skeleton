...
class ParentSiteDavDirectory extends SiteDavDirectory
{
	...
	function getChildren()
	{
		...
		if(!isset(static::$_children))
		{
			...
			{
				static::$_children[$record['Handle']] = new VirtualDavDirectory($record['Handle'], $record);
			}
		}
	...
	function getChild($name)
	{
		...
		// get children
		if(!isset(static::$_children)) $this->getChildren();
				
		if(empty(static::$_children[$name]))
			throw new Sabre_DAV_Exception_FileNotFound('The collection with name: ' . $name . ' could not be found');
	...
	function getName()
	{
		if($parentHostname = Site::$config['parent_hostname'])
